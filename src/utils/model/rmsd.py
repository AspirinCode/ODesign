...
def align_pred_to_true(
    ...
    # Fix NaN bug: clamp weighted_n_atoms to avoid division by zero when all weights are 0
    eps = 1e-12
    weighted_n_atoms = torch.clamp(
        torch.sum(weight, dim=-1, keepdim=True).unsqueeze(-1), min=eps
    )
...
# [qinghan] START add method that align according to condition
def partial_weighted_rigid_align(
    x: torch.Tensor,
    x_target: torch.Tensor,
    align_mask: torch.Tensor,
    atom_mask: torch.Tensor,
    atom_weight: Optional[torch.Tensor] = None,
    stop_gradient: bool = True,
) -> tuple[torch.Tensor]:
    """Implements Algorithm 28 in AF3 but only align condition. 
    Wrap `align_pred_to_true`.

    Args:
        pred_pose (torch.Tensor): input coordinates, it will be moved to match x_target.
            [..., N_atom, 3]
        true_pose (torch.Tensor): target coordinates for the input to match.
            [..., N_atom, 3]
        atom_weight (torch.Tensor): weights for each atom.
            [..., N_atom] or [N_atom]
        align_mask: a mask representing which coordinates to align.
            [..., N]
        atom_mask: a mask representing which coordinates to compute loss.
            [..., N]
        stop_gradient (bool): whether to detach the output. If true, will run it with no_grad() ctx.

    Returns:
        transformed_pose (torch.Tensor): rotated, translated x which should be closer to x_target.
            [..., N_atom, 3]
    """
    ...
    transformed_pose = torch.matmul(x, rot.transpose(-1, -2)) + translate
