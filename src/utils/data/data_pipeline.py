...
This module provides the DataPipeline class with static methods for:
- Loading and parsing mmCIF files
- Tokenizing protein structures
- Cropping structures to desired sizes
- Managing MSA (Multiple Sequence Alignment) features
- Managing template features
- Handling bioassembly data structures

The pipeline supports both training (WeightedPDB dataset) and distillation workflows.
"""
...
from src.utils.data.file_io import load_gzip_pickle
...
class DataPipeline(object):
    ...
    This class provides static methods for loading, tokenizing, cropping,
    and featurizing protein structure data from mmCIF files. All methods
    are static as the class serves as a namespace for data processing utilities.
    
    The pipeline supports:
    - Loading structures from mmCIF files
    - Tokenizing atom arrays into token arrays
    - Cropping structures by various methods (contiguous, spatial, interface)
    - Extracting MSA and template features
    - Managing bioassembly dictionaries
    """
    ...
    def crop(
        ...
        if focus_on_ligand:
            ref_chain_indices = [
                ref_chain_idx for idx, ref_chain_idx in enumerate(ref_chain_indices) 
                if (
                    (one_sample.mol_1_type == 'ligand' and idx == 0) or 
                    (one_sample.mol_2_type == 'ligand' and idx == 1)
                )
            ]
            ref_chain_indices = np.random.choice(ref_chain_indices, size=1)
        ...
        if focus_on_ligand:
            DataPipeline._check_lig_unchanged(
                bioassembly_dict=bioassembly_dict,
    ...
    def _check_lig_unchanged(
        ...
        selected_lig_num_before_crop = bioassembly_dict['atom_array'][
            bioassembly_dict["atom_array"].asym_id_int == ref_chain_indices[0]
        ].shape[0]

        selected_lig_num_after_crop = cropped_atom_array[
            cropped_atom_array.asym_id_int == ref_chain_indices[0]
        ].shape[0]
        
        if selected_lig_num_after_crop != selected_lig_num_before_crop:
            raise ValueError(
                f'Selected ligand is not fullly cropped, \n \
                  before crop {selected_lig_num_before_crop}, \n \
                  after crop {selected_lig_num_after_crop}, \n \
